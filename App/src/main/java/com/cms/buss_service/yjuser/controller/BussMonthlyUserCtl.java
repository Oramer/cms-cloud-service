...
public class BussMonthlyUserCtl
    ...
    public RestResult getInfo(@PathVariable("id") Long id) {
        /* 54 */
        return RestResult.ok().setData(this.bussMonthlyUserService.selectBussMonthlyUserById(id));
    }
    ...
    public RestResult remove(@PathVariable Long[] ids) {
        /* 86 */
        int rows = this.bussMonthlyUserService.deleteBussMonthlyUserByIds(ids);
        /* 87 */
        return (rows > 0) ? RestResult.ok() : RestResult.fail();
    }
}
